  ...
  for(i=0;i<itemsno;i++){
    ...
    if(rhhistory[i] !== undefined){
      ...
      if(title !== undefined){
        ...
        rh += '<a class="title" title="'+title+'" target="_blank" href="'+url+'">';
        rh += title;
        rh += '</a>';
        rh += '</span>';
        rh += '<br>';
...
window.addEvent('domready', function(){
  ...
  if($('options')){
    ...
    $('save').addEvent('click', function(){
      ...
      localStorage['clearsu'] = $('clearsu').getSelected().getProperty('value');
      
      if(localStorage['rh-itemsno'] == $('itemsno').getSelected().getProperty('value') && localStorage['rct-itemsno'] == $('rctitemsno').getSelected().getProperty('value') && localStorage['rct-itemsno'] == $('clearsu').getSelected().getProperty('value')){
