...
function getLangText(){
  ...
  switch(lang){
    case "en": return ["Recent History", "Recently Closed Tabs", "Show All History", "Clear History on Startup", "Icon Size", "Language", "(This will only clear the Recent History extension's cache not your actual history)", "Clear Recent History", "Save", "Yes", "No", "Width", "Search"]; break;
    case "fr": return ["Historique Recent", "Onglets récemment fermés", "Voir tout l'historique", "Supprimer l'historique à l'ouverture", "Taille de l'icône", "Langue", "(Cela ne supprimera que votre historique récent de votre extension et non pas votre historique)", "Supprimer l'historique récent", "Sauvegarder", "Oui", "Non", "Largeur", "Recherche"]; break;
    case "de": return ["Neuester Verlauf", "Kürzlich geschlossene Tabs", "Zeige gesamten Verlauf", "Verlauf beim Start löschen", "Symbolgröße", "Sprache", "(Es wird nur der Zwischenspeicher der Erweiterung Neuester Verlauf gelöscht, nicht der richtige Verlauf)", "Neuesten Verlauf löschen", "Speichern", "Ja", "Nein", "Breite", "Suche"]; break;
    case "es": return ["La historia reciente", "Pestañas cerradas recientemente", "Mostrar todos los Historia", "Borrar historial de inicio de", "Tamaño de los iconos", "Idioma", "(Esto sólo será evidente, no caché la extensión Historia reciente de su historia real)", "La historia reciente abierto", "Salvar", "Sí", "No", "Anchura", "Búsqueda"]; break;
    case "cs": return ["Nedávná historie", "Nedávno zavřené karty", "Ukaž celou historii", "Vymaž historii při startu", "Velikost ikon", "Jazyk", "(Tímto smažete jen vyrovnávací paměť rozšíření Nedávná historie, nikoliv svou skutečnou historii)", "Vymaž nedávnou historii", "Uložit", "Ano", "Ne", "šířka", "Hledat"]; break;
    case "ru": return ["Последние посещенные страницы", "Недавно закрытые вкладки", "Вся история", "Стирать историю при старте", "Размер иконки", "Язык", "(Очищает только историю расширения, а не фактическую историю)", "Стереть историю", "Сохранить", "Да", "нет", "ширина", "поиск"];
    default: return ["Recent History", "Recently Closed Tabs", "Show All History", "Clear History on Startup", "Icon Size", "Language", "(This will only clear the Recent History extension's cache not your actual history)", "Clear Recent History", "Save", "Yes", "No", "Width", "Search"];
  }
...
window.addEvent('domready', function(){
  ...
  if($('insert')){
    ...
    rhmain = '<div id="popup-rh-insert">'+rhmain+'</div><div id="popup-search-insert"></div>';
    ...
    var tsbar = '<span id="popup-top"><form action="#"><input id="popup-search" type="text" value="Search"><span id="popup-search-close"></span></form></span>';
    
    $('insert').set('html', uim+tsbar+rhmain);
    $('popup-search').setStyle('width', ($(document.body).getStyle('width').toInt())*1-20+'px');
