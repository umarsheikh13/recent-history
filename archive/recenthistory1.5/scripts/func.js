  ...
  for(i=0;i<itemsno;i++){
    ...
    if(rhhistory[i] !== undefined){
      ...
      if(title !== undefined && qtest){
        if(furl == 'images/icon16-other.png'){
          rh += '<div id="n'+i+'">';
          ...
          rh += '<div id="n'+i+'">';
          rh += '<a class="delitem" href="#" onclick="deleteHistoryItem(\''+what+'\', \'n'+i+'\', \''+time+'\', \''+addslashes(title)+'\', \''+url+'\', \''+furl+'\', \''+id+'\')"></a>';
          rh += '<a class="item" target="_blank" href="'+url+'">';
          rh += '<span class="time">'+time+'</span>';
          rh += '<img class="favicon" alt="FavIcon" src="'+furl+'">';
          rh += '<span title="'+title+'" class="title">';
...
window.addEvent('domready', function(){
  ...
  if($('options')){
    ...
    $('rhicon').getElement('input[value='+localStorage['rh-icon']+']').setProperty('checked','checked');
    ...
    $('save').addEvent('click', function(){
      ...
      var five = localStorage['rh-icon'] = $('rhicon').getElement('input:checked').getProperty('value');
